"""Entity resolver -- fuzzy search across ERP database for context resolution.
...
Adapted for erpclaw-web: uses db.get_erp_db() for ERP data access.
"""
...
def _get_erp_connection() -> sqlite3.Connection | None:
    """Get a read-only connection to the ERP database."""
    try:
        return get_erp_db()
...
def resolve_entity(
    ...
    Args:
        ...
        conn: Optional DB connection (if None, uses erpclaw-web's get_erp_db).
    ...
    if skill:
        ...
        if skill_entities:
            skill_conn = conn or _get_erp_connection()
            if skill_conn:
